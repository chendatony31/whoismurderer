$(document).ready(function () {
    ...
    */
    ...
    // 监听用户登出
    socket.on('note user loginout', function (userList) {
        $('#userList').html('');
        gameOver();
    ...
    // 没有收到牌
    socket.on('accept nopoker', function (data) {
        if (data[1] === nickName) {
            reNum++;
    ...
    function detectTripleCards() {
        ...
        for (i = 0; i < myPokers.length; i++) {
            ...
            for (j = i + 1; j < myPokers.length; j++) {
                if (myPokers[i] % 10 === myPokers[j] % 10) {
                    count++;
                }
            }
            if (count >= 2) {
    ...
    function isThreeOk(pokerList) {
        ...
        if (tmp_pokerList[2] - tmp_pokerList[1] === 1 && tmp_pokerList[1] - tmp_pokerList[0] === 1) {
            ...
            if (tmp_pokerList[0] % 10 === tmp_pokerList[1] % 10 && tmp_pokerList[1] % 10 === tmp_pokerList[2] % 10) {
                return true;
            }
            else {
                return false;
